...
act_container = root.find('container')
...
config_dict['image'] = act_container.find('image').text
config_dict['image_tag'] = act_container.find('image_tag').text
#------------------------------------------------------------------
...
def make_first_container():
	cmd = 'sudo docker run -dit -v Database:/acts -p '+config_dict['start_port']+':80 '+config_dict['image']+':'+config_dict['image_tag']
	result = os.popen(cmd).read()
...
def scaleup(diff):
    ...
    for i in range(diff):
        ...
        container_lock.acquire()
        temp_str = "sudo docker run -dit -v Database:/acts -p "+ str(next_port) +":80 "+config_dict['image']+":"+config_dict['image_tag']
...
def autoscaling():
    ...
    print('Auto-Scaling Thread woken up afer {} seconds.'.format(config_dict['scaling_interval']))
...
def check_container_health(container_dict):
    ...
    '''
    print("Un-healthy Containers check, Un-healthy Containers:",unhealthy_containers,file=sys.stdout)
    ...
    for port in list(unhealthy_containers.keys()):
        ...
        print("\n\n"+str(t_id),"is restarting container at port "+port)
        #sudo docker run -dit -v Database:/acts -p 8000:80 acts
        temp_str = "sudo docker run -dit -v Database:/acts -p "+ port +":80 "+config_dict['image']+":"+config_dict['image_tag']
        new_container_id = os.popen(temp_str).readlines()[0][:12]
        #print(threading.current_thread().ident)
        print(str(t_id),"has made a new container ", new_container_id ,"\n\n")
...
def categories():
	...
	if(first_request == 0):
		first_request = 1
		t2 = threading.Timer(config_dict['scaling_interval'], autoscaling)
		t2.start()
	
	no_of_requests = no_of_requests+1
	print("Request Number:",str(no_of_requests))
...
def countActs():
	...
	while(response.status_code !=200 ):
		...
		response = requests.get('http://'+config_dict['act_vm_ip_address']+':'+list(containerList.keys())[which_container]+'/api/v1/_health')
	print("Using:Port {} and Container ID {}".format(list(containerList.keys())[which_container],containerList[list(containerList.keys())[which_container]]))
	response = requests.get('http://'+config_dict['act_vm_ip_address']+':'+list(containerList.keys())[which_container]+'/api/v1/acts/count')
	which_container = (which_container+1)%len(containerList)
	return (response.text, response.status_code, response.headers.items())
